...
export function EditSheet(props : any) {
  ...
  useEffect(() => {
    if (props.editData.procedure) {
      setTestProcedures(props.editData.procedure.map((item: string) => ({ value: item })));
    } else {
      setTestProcedures([{ value: "" }]); // Default to empty if not present
    }
  
    if (props.editData.requirements) {
      setRequirements(props.editData.requirements.map((item: string) => ({ value: item })));
    } else {
      setRequirements([{ value: "" }]); // Default to empty if not present
    }
  ...
  const columnNames = useMemo(() => {
    return columns.reduce((acc: any, column) => {
      if (column.name === "procedure") {
        // Handle procedure as an array of objects with a 'value' field
        acc[column.name] = testProcedures.map(item => item.value);
      } else if (column.name === "requirements") {
        // Handle requirements as an array of objects with a 'value' field
        acc[column.name] = requirements.map(item => item.value);
      } else {
  ...
  }, [props.editData, testProcedures, requirements]);
  ...
  const submit = (e: React.FormEvent) => {
      e.preventDefault();
      form.put(route(`${props.config.route}.sqa.uat.update`,{id:props.editData.id}));
      setIsSubmit(true)
  };
