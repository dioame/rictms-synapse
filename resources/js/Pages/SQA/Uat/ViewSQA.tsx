...
export default function Index({ auth }: any) {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (value: any) => {
              ...
              return (
                <ul className="text-left list-inside">
                  {Object.entries(value.procedure).map(([key, step], index) => (
          ...
          },
          {
            ...
            render: (value: any) => {
              ...
              return (
                <ul className="text-left list-inside">
                  {Object.entries(value.requirements).map(([key, step], index) => (
                    <li key={key} className="">
                      {step ? String(step) : 'N/A'}
                    </li>
                  ))}
                </ul>
              );
        ...
        {
          key: "retesting_result",
          label: "Re-testing Result",
          className: "hidden sm:table-cell text-center",
      },
        {
          key: "actions",
    ...
    const handleDelete = (item: any) => {
        destroy(route(`${config.route}.sqa.uat.destroy`, item.id), {
        preserveScroll: true,
        onSuccess: () => {
            // toast.success(`${config.title} ${item.name} deleted successfully`);
            toast.success(`Success!`, {
            description: `${config.title} UAT deleted successfully`,
            position: "top-center",
            });
        },
        });
